...
document.querySelector('.section-action').addEventListener('click', function (e) {
  ...
  if (e.target.classList.contains('modal-btn')) {
    ...
    window.stop();//Ne radi sa Edge meščini, niti Chrome
    let url;
    let naslov;
    fetch('http://localhost:3000/artikli')
      ...
      .then(function (myJson) {
        ...
        `;
        // let artikal = JSON.stringify(kupljeniArtikal);
        // console.log(JSON.parse(artikal));
...
function fetchBoughtArticles(){
  // let dropdownCart = document.querySelector('.dropdown-column');
